...
* O conteúdo de um componente é alterado na instância do Autor e ativado; ele não ficará visível na instância do Publish até que o componente seja reiniciado **com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489 e NPR-32232).
...
>[!NOTE]
>
>Os pacotes complementares do AEM Forms ajudam a alinhar a funcionalidade dos AEM Service Packs e dos Cumulative Fix Packs. Portanto, é essencial instalar o pacote complementar do AEM Forms após instalar qualquer AEM Service Pack, Cumulative Fix Pack ou Feature Pack.
...
* O Assets foi selecionado (na exibição em Coluna, na interface para toque) antes de selecionar a opção de filtro na lista suspensa Somente conteúdo e, em seguida, selecionar a opção de movimentação também são movidos (NPR-30693).
...
* Um usuário se concentra nos campos do painel à esquerda e usa um atalho de teclado para colar o conteúdo. Ao fazer isso, ele cola o conteúdo da área de transferência do editor de páginas em vez do conteúdo copiado dos campos do painel à esquerda (NPR-31173).
...
* Os relatórios do Analytics não são preenchidos no ambiente de autor do AEM. Em vez disso, uma página em branco é exibida (NPR-30905).
...
* Não é possível abrir a descompactação de um arquivo compactado com uma pasta que tenha um sinal de porcentagem (%) no nome usando a interface do Assets. NPR-29989: Hotfix do CQ-4270467
...
* Ao gerar uma exportação de arquivos CSV, a adição de colunas personalizadas à Exibição de lista interrompe o relatório. NPR-30208: Hotfix do CQ-4273344
* Os relatórios OOTB em /etc/reports/ não estão funcionando corretamente e o gráfico de dados históricos não é exibido. NPR-30016: Hotfix do CQ-4220180
* O valor do parâmetro de solicitação resourceType é copiado no valor de um atributo de tag HTML que é encapsulado em aspas duplas. NPR-29832: Hotfix do CQ-4255365

### Communities {#communities-1}

* Problema de conteúdo duplicado com o console de moderação do AEM Communities. NPR-30667: Hotfix do CQ-4276829
...
* O upload de um arquivo de arte do Adobe Illustrator com mais de 2 GB gera um erro de espaço de heap do Java™. NPR-29265: hotfix do CQ-4226217
...
* O CoralUI, quando usado com Multifield, armazena fileReferenceParameter no nível do componente em vez do nível de vários campos. NPR-29535: Hotfix do CQ-4266129
* Problema ao tentar comparar a versão de página mais recente e atual no AEM 6.3.3.3. NPR-29532: Hotfix do CQ-4269639
* O campo de caminho é aberto no caminho raiz independentemente do caminho especificado para pesquisa. NPR-29398: Hotfix do CQ-4268897
...
* Um alerta de erro é gerado ao adicionar componentes à página WCM quando a minificação é ativada na instância. NPR-29476: Hotfix do CQ-4266197
...
* Não é possível exportar Fragmentos de experiência para o target quando uma imagem inteligente é usada. Hotfix do CQ-4269606

### Replicação {#replication}

* O componente Agente de replicação é susceptível a uma vulnerabilidade que exibe informações confidenciais para usuários não autorizados. NPR-29613: Hotfix do Granite-25070
...
* O CommandLineProcess do fluxo de trabalho falha com um nome de arquivo com aspas simples. NPR-28805: Hotfix do CQ-4262287
...
* Não é possível excluir representações de imagem de um ativo de vídeo da página de detalhes do ativo. NPR-29149: Hotfix do CQ-4266073
...
* Usar um componente do Target com um Contêiner de layout aninhado e um componente de texto aciona um erro JavaScript &quot;Não é possível ler a propriedade currentPos de nulo&quot; ao editar o texto ou clicar no contêiner. NPR-29077: Hotfix do CQ-4246594
* (Interface para toque) Não é possível atualizar tags em massa para páginas que já estão marcadas com tags diferentes. NPR-28729: Hotfix do CQ-4262922
* Abrir a variação na Exibição de cartão causa um erro 500. NPR-28611: Hotfix do CQ-4263571
* Implantação de uma estrutura que foi movida em um cliente potencial primário para um `cq:moveTarget` errado. NPR-28968: Hotfix do CQ-4265280
...
* Ativada a opção para selecionar itens por página na página Exibição do conjunto de políticas.
...
* Uma tarefa de resposta de fila compartilhada abre um elemento flash no espaço de trabalho do HTML5. NPR-29161: Hotfix do CQ-4266498
* Não é possível enviar do Workspace se o botão contiver um caractere de trema. NPR-29014: Hotfix do CQ-4263172
...
* Há um comportamento diferente ao visualizar um XDP como PDF versus o mesmo XDP como HTML no Designer. NPR-28602: Hotfix do CQ-4260239
...
* Adição do aria-label à lista e ao botão Exibição de cartão.
...
* Adição do aria-label à lista e ao botão Exibição de cartão. NPR-27736: Hotfix do CQ-4254027
...
* Erro ao procurar caracteres como `(`, `+` e `?` na pesquisa rápida. NPR-28212: hotfix do CQ-4260969
* Não é possível excluir comentários no conteúdo gerado por usuários usando a API. NPR-28075: Hotfix do CQ-4260534
* Os comentários postados na próxima página são destacados com amarelo quando um novo comentário é postado. NPR-28148: Hotfix do CQ-4259681
...
* Não é possível definir uma política de modelo em Parsys aninhados sem configurá-la no componente principal. NPR-27987: hotfix do CQ-4246095
...
* O ResourceResolver não resolve vários domínios para o componente de Destino. NPR-28265: Hotfix do CQ-107847
* O cancelamento da herança da Live Copy agora funciona corretamente em contêineres direcionados, à medida que as ações são exibidas. NPR-28129: Hotfix do CQ-4259813

#### Replicação {#replication-1}

* DispatcherFlushRules interrompeu a replicação na 6.3.3.1 NPR-28150: Hotfix do CQ-4261401
...
* Adição do suporte à guia Regras e sua imposição na pasta de ativos no esquema de metadados da pasta.
...
* A consulta foi aprimorada para que use o recurso ExpiryNotification. NPR-26999: Hotfix do CQ-4251188
...
* O ajuste de referência de ativo não atualiza referências para ativos que fazem parte de Sling ResourceCollections. NPR-26759: Hotfix do CQ-4252605
* O download por meio do email de compartilhamento de link retorna um arquivo zip vazio ou corrompido. NPR-27997: Hotfix do CQ-4259686
* A codificação de vídeo híbrido não é concluída e nenhuma miniatura é criada. NPR-27122: Hotfix do CQ-4255080

#### Sites {#sites-6}

* Suspender a página principal remove o tipo de mixagem cq:LiveRelationship da página ausente. NPR-26996: hotfix do CQ-4254113
...
* Quando o Editor de Rich Text é combinado com vários campos, o erro Uncaught TypeError: fieldAPI.getName não é uma função no foundation.js ocorre. NPR-27146: Hotfix do CQ-4253155/CQ-4259967
...
* Uma imagem não é exibida após especificar pontos de interrupção em um componente do Dynamic Media. Hotfix do CQ-4256168
...
* O link para o modelo de Fragmento de conteúdo está ausente após a aplicação do patch do SP3. Hotfix do CQ-4259029

#### DAM - IU {#dam-ui}

* A interface do esquema de metadados de pasta está quebrando após a instalação do SP3. Hotfix do CQ-4257737
...
* O link para respostas não está funcionando no clique de linha. NPR-24556: Hotfix do CQ-4256138
...
* O rótulo do status de conclusão do recurso de Scorm da ativação aparece vazio na interface. NPR-27152: Hotfix do CQ-4249994
* Ao habilitar **Permitir privilegiado**, os membros privilegiados só devem poder compor para usuários que sejam membros da comunidade. NPR-27901: Hotfix do CQ-4251235
...
* Salvar e fechar nas propriedades da página não retorna à página apropriada na GUERRA contra o AEM com Tomcat. NPR-27567: hotfix do Granite-23671

* O texto inserido é perdido pelo recurso sourceEdit após salvar. Hotfix do CQ-4259273
...
* Atualização do Construtor de formulários para a versão mais recente do Handlebars.
...
* Com a dissociação de IDS ativada, o fluxo de trabalho Atualizar ativo do DAM não vincula mais referências. NPR-26135: Hotfix do CQ-4250933
* Não é possível abrir a descompactação de um arquivo criado pela etapa de desarquivamento com uma pasta chamada com um %. NPR-26275: Hotfix do CQ-4251482
* Um caractere de aspas simples impede a atualização dos metadados. NPR-26808: Hotfix do CQ-4254305
...
* O Compartilhamento de link de ativo não permite adicionar mais de um usuário interno com uma ID de usuário numérica. NPR-26206: Hotfix do CQ-4251466
* Corrupção em arquivos zip compactados com o algoritmo deflate64. NPR-26793: Hotfix do CQ-4253995
* O processo de geração de miniaturas não funciona corretamente em arquivos de PDF complexos, resultando em miniaturas, em que parte da imagem está ausente. NPR-26057: Hotfix do CQ-4250944
...
* A opção **Excluir representação** não funciona e emite um erro &quot;nada a excluir&quot;. NPR-26007: Hotfix do CQ-4213414
...
* Os campos de tag do modelo de Fragmento de conteúdo estruturado não refletem as operações de movimentação de tag. NPR-26801: Hotfix do CQ-4251805
...
* Criar uma variação como Live Copy requer que o usuário atualize a página para refletir. NPR-26663: Hotfix do CQ-4254328
...
* A replicação usa o token expirado para OAuth, causando vários erros. NPR-25894: Hotfix do GRANITE-22388
...
* As imagens do anexo não mostram uma miniatura ao criar uma publicação no fórum. NPR-26380: Hotfix do CQ-4253105
* O plug-in Colar do Word não carrega mais de uma imagem. NPR-26728: Hotfix do CQ-4253638
...
* A recuperação de dados do Provedor de Recursos de Armazenamento Adobe é lenta. NPR-26237: Hotfix do NPR-24152
* A página de composição da mensagem privada é irregular e lenta. NPR-26120: Hotfix do CQ-4250923
...
* A exclusão da publicação funciona, mas gera um erro no console e a publicação permanece visível na interface do usuário. NPR-26292: Hotfix do CQ-4252803
...
* Não é possível criar ou editar as configurações de marca d&#39;água da Segurança de documentos. NPR-26267, NPR-26129: Hotfix do CQ-4250234
...
* Editar e salvar um módulo editável e, em seguida, abrir ou fechar a Visualização de PDF, mantém a Visualização de PDF e HTML da Carta visível. Ambas as visualizações permanecem funcionais na mesma janela. NPR-26770: Hotfix do CQ-4253217
...
* Uma exceção é lançada ao serem definidos modelos de pesquisa na interface do administrador e usados para pesquisar no espaço de trabalho do AEM Forms. Hotfix do CQ-4255649
* Os detalhes de rastreamento dos processos nos aplicativos com o símbolo de parênteses no nome não são exibidos no HTML Workspace. Hotfix do CQ-4242101
* Salvar alterações na tela de preferências do HTML Workspace aciona uma exceção. Hotfix do CQ-4241485
...
* Falha ao carregar esapi.properties como um recurso de carregador de classe em WAS9. Hotfix do CQ-4255631
...
* Correção dos problemas relatados no relatório de análise de código estático do PDFG. NPR-23251: Hotfix do CQ-4213930
...
* Alterado o comprimento do tipo de campo para 255 para suportar todos os tipos MIME possíveis para os diferentes tipos de anexos.
...
* Atualize com.adobe.granite.ui.coralui3-rte para a versão mais recente lançada.
* Verificado se a condição de renderização retorna um resultado seguro se expressionResolver for nulo.
...
* Incompatibilidade no tamanho do download em comparação às informações exibidas. NPR-23418: Hotfix do CQ-4242774
...
* O botão Publish/Desfazer publicação no Brand Portal não deve estar disponível para Fragmentos de conteúdo. NPR-23318: Hotfix do CQ-4245086
* (Brand Portal) Habilite a criação de ativos secundários quando um ativo é publicado. NPR-23331: hotfix do CQ-4242018
* As solicitações do Dynamic Media não usam um cliente comum proxy/HTTP. NPR-10727: Hotfix do CQ-45695, CQ-88800
* Não é possível anotar um único ativo de vídeo MP4 de representação no Dynamic Media S7 (DMS7). NPR-22046: Hotfix do CQ-4215912
...
* Problemas de seleção/exibição da Representação dinâmica com uma grande contagem de Predefinições de imagem. NPR-23151: Hotfix do CQ-4217511
* Problema com a codificação de vídeo do Dynamic Media - iniciador de modificação/reupload. NPR-23237: Hotfix do CQ-4240260
* Correção de manuseio de proxy para o encaminhador HTTP no Dynamic Media S7. NPR-24001: Hotfix do CQ-244140

### Sites {#sites-8}

* Discrepâncias do Query Builder resultantes de diferentes traduções do xPath entre as versões 6.2 e 6.3. NPR-23245: Hotfix do CQ-4240396
...
* Corrija a opção que pode ser reordenada na Exibição em lista. NPR-23065: hotfix do CQ-4239321
...
* O botão Implantação está ausente na guia Blueprint para Variações XF. NPR-23320: Hotfix do CQ-4240404
...
* Quando aproximadamente Adicionamos 80 fragmentos em uma pasta no Assets e erros são encontrados quando o fluxo de trabalho é acionado no console Linha do tempo. NPR-23393; Hotfix do CQ-4211216
...
* A coleção inteligente não exibe resultados após salvar se estiver usando o predicado de tags na faceta de pesquisa. NPR-23401: Hotfix do Granite-21278
* Patch jQuery 1.12.4 do clientlib para incluir a correção de segurança. NPR-24128: Hotfix do Granite-20058
* As traduções de internacionalização agora são atualizadas quando o pacote é reiniciado. NPR-23193: Hotfix do Sling-7190
...
* (Target) As campanhas não são ativadas ao usar um tipo de API como XML. NPR-23199: Hotfix do CQ-4240936****
* A replicação de referência de configuração falha com uma estrutura de pasta intermediária. NPR-23485: Hotfix do CQ-4242751
...
* DatePicker não dá suporte à dica de tipo externo definida manualmente por um campo oculto. A alteração da dica de tipo aciona um erro de conversão. NPR-23371: Hotfix do Granite-21194
* Coral.ColumnView: adiciona suporte para shift+click. NPR-23404: Hotfix do Granite-13338
* Selecionar RT não valida quando um item com um valor vazio é selecionado. NPR-23405: Hotfix do Granite-21283
...
* A pesquisa em comunidades está falhando por poucas palavras. NPR-23256: Hotfix do CQ-4240717
...
* Os motivos de sinalização de usuários que não estão sinalizados permanecem visíveis. NPR-23974: hotfix do CQ-4243822
...
* Solicitação para que um mecanismo limpe os ativos das instâncias do trabalhador após o processamento ou regularmente. NPR-23638: hotfix do Granite-21337
...
* Um campo numérico não respeita um valor mínimo. NPR-23967: Hotfix do CQ-4244830
...
* Atualização dos pacotes de Modelos do Sling para incluir a correção de conjunto de caracteres.
...
* Ao alterar o título ou a imagem em miniatura nas configurações da pasta do ativo, o grupo original e as permissões da pasta são substituídos. NPR-22171: Hotfix do CQ-4216080
* A interface do usuário emite um erro falso &quot;Falha do Publish para o Brand Portal&quot;, enquanto o trabalho é adicionado à fila Replicação e os ativos são publicados no Brand Portal. NPR-22179: Hotfix do CQ-4205273
* (Interface de toque) Local de upload padrão para ativos na exibição em Coluna. NPR-22465: Hotfix do CQ-4237057
* AEM lança um erro StackOverflow ao tentar copiar um esquema de ativo de `/conf/global` para `/conf/mytenant`. NPR-22489: Hotfix do CQ-4235875
...
* Tornar o compartilhamento público como a opção padrão ao publicar ativos no Brand Portal. NPR-21931: Hotfix do CQ-4218816
...
* Os componentes de estrutura editáveis perdem os nomes de classe CSS necessários para a grade responsiva quando editados. NPR-21741: Hotfix do CQ-4232374
* (Interface para toque) Várias vulnerabilidades de Script entre sites (XSS) para componentes HTL. NPR-21899: Hotfix do CQ-4232511
...
* A tela cheia do RTE para a caixa de diálogo da interface para toque está ocultando os plug-ins do RTE. NPR-22034: Hotfix do CQ-4235457
...
* Problemas de persistência quando os armazenamentos do ContextHub substituem a camada de persistência padrão durante a inicialização. NPR-22479: Hotfix do CQ-4218399
...
* Após virar uma imagem adicionada ao componente de imagem em uma página, as alterações não são salvas e a imagem original é exibida na página. O suporte à renderização foi adicionado ao componente principal da imagem por meio de [https://github.com/adobe/aem-core-wcm-components/pull/141](https://github.com/adobe/aem-core-wcm-components/pull/141).  NPR-22801: Hotfix do CQ-4221539
* Quando o usuário tenta excluir a âncora existente do menu de âncora, a janela do componente Editor de rich text é fechada e as alterações permanecem não salvas. NPR-22802: Hotfix do CQ-4238167
...
* Ativar uma página com instâncias de publicação de resultados em um status verde em vez de âmbar. NPR-22927: hotfix do CQ-4236310
...
* O mecanismo do Target (mbox.js, at.js) não usa URLs danificados. E o usa URLs contendo dois pontos que podem falhar com determinadas implantações. NPR-22366: Hotfix do CQ-4237854
...
* A definição de metas do AEM falha ao usar o idioma diferente do inglês. NPR-22917: Hotfix do CQ-4218213
...
* O iParsys cria o deslocamento de posição depois de desmarcar a caixa de seleção cancelar/desativar herança. NPR-21905: Hotfix do CQ-4230951
...
* (Interface Clássica) O componente exibe os valores padrão mesmo se o serviço de Modelo de dados de formulário associado estiver definido como um campo vazio. NPR-22272: Hotfix do GRANITE-19744
...
* O LogbackManager pode perder algumas configurações OSGi no momento da inicialização. NPR-23060: Hotfix do Granite-19791
...
* Adição de suporte para produzir arquivos de PDF compatíveis com PDF/A-2a ou PDF/A-3a.

### Pacote complementar do Forms {#forms-add-on-package-9}

#### Forms - Interface do usuário de administração {#forms-admin-ui}

* A senha é visível em texto simples quando você inspeciona o campo de senha nas páginas de Configuração dos Conectores ECM. NPR-22508: Hotfix do CQ-4236065
...
* As funções personalizadas do Editor de regras estão adicionando um; extra após a chamada de função. Portanto, eles estão falhando na validação mesmo que a função personalizada retorne como verdadeiro. NPR-22481: Hotfix do CQ-4235499
* Independentemente do padrão de data selecionado, o componente Seletor de datas não segue o padrão ao exibir mensagens de validação mínimas e máximas. NPR-22444: Hotfix do CQ-4236269
* O formato de data que está sendo enviado na solicitação de envio deve estar alinhado ao padrão fornecido no componente seletor de datas. NPR-22384
...
* O componente de Contêiner do AEM Forms não estará disponível para edição depois que o formulário errado for configurado na página do AEM Sites. Hotfix do CQ-4237456
* Os testes de desenvolvimento estão falhando quando executados em servidores JEE. Hotfix do CQ-4222082
...
* As Forms modificadas após a publicação da página não são publicadas novamente. NPR-23013: Hotfix do CQ-4236566
...
* (HTML Workspace) A preparação de dados é acionada ao fechar um formulário no espaço de trabalho. NPR-21127: Hotfix do CQ-4224574
...
* Adição de suporte para produzir arquivos de PDF compatíveis com PDF/A-2a ou PDF/A-3a. NPR-23021: Hotfix do CQ-4214172

#### Designer do Forms {#forms-designer-1}

* O identificador PDF/UA está ausente quando salvo como PDF do Designer. NPR-23137
...
* Os hiperlinks incorporados definidos no Designer não são marcados ou apresentam texto alternativo quando salvos como PDF do Designer. NPR-23133
...
* Exceção encontrada ao assinar digitalmente um documento PDF por meio do HSM. NPR-21154: Hotfix do CQ-4226978
...
* Não é possível baixar ativos com espaços na página de verificação de licença. NPR-20067: Hotfix do CQ-4216557
...
* Muitos caracteres no OmniSearch podem causar falha no servidor AEM. NPR-20434: hotfix do CQ-4223602
...
* O AEM Dynamic Media Classic (antigo Scene7) não sincroniza ativos do servidor AEM. Esse problema ocorre quando o caminho de destino na Configuração de Cloud Service aponta para uma subpasta do caminho raiz. CQ-4228265
...
* O nome do fluxo de trabalho digitado deve estar disponível na caixa Notificação ao publicar uma página por meio de Gerenciar publicação. NPR-20046: Hotfix do CQ-4221586
...
* O plug-in de Rich text não funciona nas barras em linha e de tela cheia na interface para toque de vários campos. NPR-20973
...
* A configuração do Search &amp; Promote Cloud Service ignora o método de recuperação de formulário, tornando-o inutilizável na instância do autor. NPR-20447: Hotfix do CQ-4206076
* A definição ambígua de filtro no pacote de conteúdo faz com que os caminhos sejam substituídos quando o recurso Search &amp; Promote for instalado. NPR-20808

### Mobile On-Demand {#mobile-on-demand}

* As propriedades de metadados para ativos do tipo TXT são exibidas em diferentes editores de metadados sempre que suas propriedades são exibidas. NPR-20239
...
* requestAttributes agora não está definido para as outras instruções data-sly-resource. NPR-20639: Hotfix do 4226206
* Correção de problemas com solicitações curl Head para ativos OOTB no AEM. NPR-20781: Hotfix do CQ-4221520

### Projetos {#projects-1}

* O acesso a diferentes projetos do console Projetos demora mais para carregar. NPR-20314
...
* O campo Cor é definido como &quot;sempre obrigatório&quot; apesar dos atributos na caixa de diálogo. NPR-19702
...
* Não é possível definir o valor do componente Switch entre verdadeiro ou falso. NPR-21111
...
* O nome de um objeto de formulário XDP como um subformulário é exibido como a dica de ferramenta quando não está definido nas configurações de acessibilidade. NPR-20523
...
* Ativação da ação Publicar na Brand Portal para o formulário de esquema de metadados padrão.
* Alteração de comportamento na exibição de títulos no cartão de Imagem para Imagem com a propriedade dc: title, definida como `String [] (multifield)`.
* Renderização de tempo do lado do servidor substituída por um componente de tempo de base.
* Ativação da opção Publicar tags do AEM para o Brand Portal no console tagadmin/tagging.
...
* Mapear dois campos com a mesma propriedade com tipos diferentes de campo de propriedade gera um erro interno. NPR-19462: HF do CQ-4216828
...
* Não é possível carregar uma lista completa de páginas/ativos na exibição em Coluna usando o Chrome. NPR-19566: Hotfix do CQ-4214248
...
* A seção de público-alvo é corrompida ao criar a experiência. NPR-19110
...
* WCMMode.EDIT não funciona para componentes direcionados no modo Criação. NPR-19387
...
* A renderização de problemas com o console de artigos AEM como o uso de imagens em tamanho real o torna lento. NPR-19088
...
* A biblioteca de tags personalizada que faz referência a tags implementadas como script não é encontrada após a atualização para o AEM 6.3. NPR-19087
...
* Os editores de projeto não podem copiar e colar ativos na pasta de ativos do projeto. NPR-19619: hotfix do CQ-4215321

### Gerenciamento de conteúdo da Web {#web-content-management}

* Na tela Implantação, as caixas de seleção correspondentes às páginas de Live Copy não podem ser marcadas ou desmarcadas. NPR-19518
...
* As propriedades OOTB e de alinhamento de imagem não são exibidas ao editar uma imagem na interface clássica. NPR-19488: HF do CQ-4217914
...
* O fluxo de trabalho Enviar para JEE não retorna o parâmetro de saída do JEE para o AEM e lança o erro &quot;Ignorar porque o valor não é primitivo&quot;. NPR-20265
...
* Ao configurar o Google `reCaptcha` usando o Cloud Service, em ambientes que exigem a configuração de org.apache.http.proxyconfigurator para conexões externas, as chamadas de POST não parecem passar pelo PROXY. NPR-20454
* O Forms adaptável baseado em esquemas XSD envia valores XML com falha para campos numéricos em instalações com localidades que têm um separador decimal diferente de ponto (.) resultando em erros. NPR-20444
* As configurações de proxy definidas para &quot;Configuração de proxy de componentes HTTP Apache&quot; não são respeitadas ao fazer uma solicitação HTTP em um servidor de terceiros. Problemas de tempo limite de conexão usando chamadas HTTP GET ou POST. NPR-20457, NPR-20456, NPR-20455, NPR-20451
...
* O campo de texto multilinha (TextArea) não retém caracteres de nova linha ou quebras no texto após enviar dados no HTML Workspace. NPR-20085
...
* O campo suspenso Esquema de metadados não pode ser transformado em obrigatório, pois a validação obrigatória de campo está ausente para ativos. NPR-17882: FP do CQ-4208373
...
* Solução de problemas de validação do fragmento de conteúdo
* Aprimoramento da capacidade de edição dos esquemas de metadados em dispositivos híbridos
...
* A tarefa de revisão de ativos na pasta de idioma japonês não funciona. CQ-4204782
* Uma exceção de ponteiro nulo ocorre quando um ativo é movido de sua página de propriedades. CQ-4204251
...
* Problemas com a adição de novas guias no formulário de Esquema de metadados quando ele é editado no Chrome em dispositivos híbridos. CQ-4201810
...
* Os usuários com permissões somente leitura para o aplicativo não podem visualizar o conteúdo do gerenciamento de conteúdo do AEM Mobile. NPR-17486: CFP do CQ-4209690
* O botão Criar catálogo não funciona na exibição de coluna no console Catálogo. CQ-4209952
...
* Os componentes localizados modificados não são restaurados ao formulário original quando a herança é reaplicada em uma Live Copy. NPR-18172: Hotfix do CQ-4211379
...
* O nome da página não é gerado de acordo com a convenção para títulos de página longos. NPR-17633: Hotfix do CQ-4209056
...
* Ocorre um erro de data não analisável ao editar uma tarefa na caixa de entrada. CQ-4208749
...
* Correções nos módulos de texto de gerenciamento de correspondência, pré-visualizações de correspondência e inicialização programática para criar uma interface do usuário de gerenciamento de correspondência.
* Correções da validação de PDF/A-1b, conversão de arquivos de imagem grandes em PDF e documentos PDF do idioma japonês no Gerador de PDF.
* Correções de usabilidade do gerenciamento de correspondência, da Segurança de documentos e do Forms Workflow.
...
* Ao editar um fragmento de gerenciamento de correspondência, o Editor de texto exibe condições em linha junto com o texto processado. CQ-4211930
* Ao criar uma carta de gerenciamento de correspondência, a descrição da carta não é salva. NPR-18089
* A margem extra acima e abaixo de uma lista com marcadores é visível no Editor de texto, mas não na representação HTML e PDF. NPR-18126
...
* O serviço PDF Generator não converte imagens com mais de 100 MB em documentos PDF. CQ-4208628
* Ao usar o serviço PDF Generator com um OCR em japonês, é gerado um PDF invertido. NPR-17602
...
>[!NOTE]
>
>A Adobe recomenda que você execute a validação em cada novo pacote de instalação antes de tentar instalá-lo. A pré-validação analisa e relata os erros encontrados antes da instalação, além de avisar os usuários sobre esses erros de forma proativa.
>
>Você pode acessar a documentação da opção Validar em [https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)

* O AEM 6.3.3.0 é um pré-requisito do CFP. Consulte a [Documentação de Atualização](https://experienceleague.adobe.com/br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) para obter instruções detalhadas sobre como atualizar uma instalação de AEM para AEM 6.3.
...
* Antes de instalar o Cumulative Fix Pack, faça um instantâneo ou um backup da sua instância do AEM.
...
* Os seguintes erros e avisos podem ocorrer durante a instalação do AEM CFP 6.3.3.x e podem ser ignorados com segurança:
   ...
   * org.apache.sling.engine.impl.SlingRequestProcessorImpl Serviço ServletResolver ausente, não é possível fazer solicitações de serviço, envio do status 503
...
* A partir da versão 6.3.1.2, o botão Publish para Brand Portal das coleções inteligentes foi removido.
...
Entidade de segurança `b)` : reference-adjustment-service
Tipo: Permitir
Privilégios : jcr:read, jcr:modifyProperties
Restrições : rep:glob=&quot;/&#42;/jcr:content/&#42;&quot;
...
O valor padrão é 1. Substitua o valor ao sobrepor o arquivo constantes.js (`/libs/cq/ui/widgets/source/constants.js`), cancelando o comentário da propriedade `CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL`. Em seguida, atribua a ele o valor necessário (a profundidade máxima sob `/jcr:content` da página, onde são armazenados os dados das propriedades da página).
...
Esse processo permite que o sling GET servlet retorne todos os nós necessários.

## Uber Jar {#uber-jar}

O Uber Jar da versão 6.3.3.8 está disponível no repositório do Adobe Public Maven

Para usar o Uber Jar em um projeto Maven, consulte [Como usar o Uber Jar](https://experienceleague.adobe.com/br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) e inclua a seguinte dependência no POM do projeto:
...
|----|-----|-----|-----|
| Integração do Assets e da Adobe Creative Cloud | [O compartilhamento de pastas do AEM para a Creative Cloud](https://experienceleague.adobe.com/br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) foi introduzido no AEM 6.2 como uma forma de fornecer aos usuários criativos acesso aos ativos do AEM. Um novo recurso lançado no aplicativo Creative Cloud, o Adobe Asset Link, fornece uma melhor experiência ao usuário. Ele também fornece um acesso mais eficiente aos ativos do AEM diretamente da Photoshop, do InDesign e do Illustrator.</br></br> A Adobe não fará mais aprimoramentos no recurso de compartilhamento de pastas. Embora o recurso esteja incluído no AEM, os clientes são aconselhados a usar a substituição. | Adobe Asset Link ou Aplicativo de desktop. Para obter mais informações, consulte o artigo [integração de Creative Cloud do AEM](https://experienceleague.adobe.com/br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). | AEM 6.3.3.x |
...
>* Inscreva-se em [Atualizações de produtos de prioridade Adobe](https://www.adobe.com/subscription/priority-product-update.html)
