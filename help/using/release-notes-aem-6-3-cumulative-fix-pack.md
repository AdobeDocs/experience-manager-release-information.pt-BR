...
* O componente Container do AEM Forms fica invisível assim que o formulário errado é configurado na página do AEM Sites e a caixa de seleção “formulários cobrem a largura total da página” é selecionada. NPR-25972: Hotfix do CQ-4239287, CQ-4249133
...
* A exceção de ponteiro nulo é acionada quando o ponto inicial associado ao conjunto formulários ou um formulário adaptável não pré-preenchido é aberto. Hotfix do CQ-4256124
...
* O comprimento do tipo de campo foi alterado para 255 para suportar todos os tipos de MIME possíveis dos diferentes tipos de anexos.
...
* Ao executar vários lançamentos, o lançamento é promovido no autor, mas as alterações não são replicadas no servidor de publicação devido à falta de permissões de replicação. NPR-22934: Hotfix do CQ-4234746
...
* A filtragem do campo Tag não está funcionando conforme o esperado ao digitar texto. NPR-22236: Hotfix do CQ-4236655
...
* PersonalizationDataSource aciona uma Exceção de ponteiro nulo devido à ausência do nó `jcr:content`. NPR-22850: hotfix do CQ-4222122
...
* O componente de container do AEM Forms não está disponível para edição quando o formulário errado é configurado na página do AEM Sites. Hotfix do CQ-4237456
...
* O plug-in de rich text não funciona nas barras em linha e tela cheia na interface do usuário de toque de vários campos. NPR-20973
...
* O campo de texto multilinha (TextArea) não retém caracteres de novas linhas ou quebras de texto após enviar dados no espaço de trabalho HTML. NPR-20085
...
O valor padrão é 1. Substitua o valor sobrepondo o arquivo constants.js (`/libs/cq/ui/widgets/source/constants.js`) cancelando o comentário da propriedade CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL e atribuindo o valor necessário (a profundidade máxima sob `/jcr:content` da página onde os dados das propriedades da página são armazenados).
